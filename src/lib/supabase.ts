...
// Выводим предупреждение в консоль, если используются значения по умолчанию
if (process.env.NEXT_PUBLIC_SUPABASE_URL !== DEFAULT_SUPABASE_URL ||
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY !== DEFAULT_SUPABASE_ANON_KEY) {
  console.log('Используются переменные окружения Supabase из .env файлов')
} else {
  console.warn('Используются значения по умолчанию для Supabase. Проверьте переменные окружения.')
}
